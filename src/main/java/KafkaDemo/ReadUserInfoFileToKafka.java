...
public class ReadUserInfoFileToKafka {
    ...
    public static Logger logger = LoggerFactory.getLogger(ReadUserInfoFileToKafka.class);
...
//        }
        ...
        if(!new File(krb5conf).exists()) {
            krb5conf = ReadUserInfoFileToKafka.class.getClassLoader().getResource("krb5.conf").getPath();
        }
        String jaasconf = confPath + File.separator + "jaas.conf";
        if(!new File(jaasconf).exists()) {
            krb5conf = ReadUserInfoFileToKafka.class.getClassLoader().getResource("jaas.conf").getPath();
        }
        ...
        String filePath = confPath + File.separator + "UserInfo.txt";
    ...
    private static  Properties getPublicProps() {
        ...
        try{
            File file = new File(confPath + File.separator + "app.properties");

            if(!file.exists()) {
                InputStream in = ReadUserInfoFileToKafka.class.getClassLoader().getResourceAsStream("app.properties");
                properties.load(in);
            } else {
